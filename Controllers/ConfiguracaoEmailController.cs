...
namespace Gerente.Controllers
{
    public class ConfiguracaoEmailController : Controller
    {
        ...
        private ConfiguracaoEmail ObterConfiguracaoEmail()
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT id, servidor_smtp, porta, email_remetente, nome_remetente, usuario_smtp, senha_smtp, data_criacao, data_atualizacao FROM configuracoes_email ORDER BY id DESC LIMIT 1", conn))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            return new ConfiguracaoEmail
                            {
                                ...
                                SenhaSmtp = reader.IsDBNull(reader.GetOrdinal("senha_smtp")) ? "" : reader.GetString(reader.GetOrdinal("senha_smtp")),
        ...
        private void SalvarConfiguracaoEmail(ConfiguracaoEmail configuracao)
        {
            ...
            using (var conn = new NpgsqlConnection(connString))
            {
                ...
                if (configuracao.Id > 0)
                {
                    ...
                    using (var cmd = new NpgsqlCommand("UPDATE configuracoes_email SET servidor_smtp = @servidor, porta = @porta, email_remetente = @email, nome_remetente = @nome, usuario_smtp = @usuario, senha_smtp = @senha, data_atualizacao = CURRENT_TIMESTAMP WHERE id = @id", conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@senha", string.IsNullOrEmpty(configuracao.SenhaSmtp) ? DBNull.Value : configuracao.SenhaSmtp);
                        cmd.ExecuteNonQuery();
                    }
                }
                else
                {
                    ...
                    using (var cmd = new NpgsqlCommand("INSERT INTO configuracoes_email (servidor_smtp, porta, email_remetente, nome_remetente, usuario_smtp, senha_smtp) VALUES (@servidor, @porta, @email, @nome, @usuario, @senha)", conn))
                    {
                        cmd.Parameters.AddWithValue("@servidor", string.IsNullOrEmpty(configuracao.ServidorSmtp) ? DBNull.Value : configuracao.ServidorSmtp);
                        cmd.Parameters.AddWithValue("@porta", configuracao.Porta);
                        cmd.Parameters.AddWithValue("@email", string.IsNullOrEmpty(configuracao.EmailRemetente) ? DBNull.Value : configuracao.EmailRemetente);
